...
import {
  ...
  export interface ProxyOptions {
    ...
    // Callback function to finalize the stream and call the onFinalize callback 
  ...
  export async function proxySSE(opts: ProxyOptions): Promise<Response> {
    ...
    // If the upstream response is not OK or has no body, return an error response 
    ...
    // Create a downstream readable stream  
    const downstream = new ReadableStream<Uint8Array>({
        async start(controller) {
            ...
            // Set up a heartbeat interval to send heartbeat comments every N ms 
            ...
            // Function to finalize the stream and call the onFinalize callback 
            ...
            try {
                // Read the upstream response in a loop 
                while (true) {
                    const { done, value } = await reader.read(); 
                    if (done) break; 

                    // Decode the upstream response and split it into events and rest 
                    ...
                    for (const block of events) {
                        for (const data of extractDataLines(block)) {
                            ...
                            try {
                                ...
                                lastText += piece;
                                chunks += 1;
                                controller.enqueue(enc.encode(encodeSseData(json)));
                            } catch {
                                chunks += 1;
